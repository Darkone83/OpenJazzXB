...
int MainMenu::main() {
	...
	/* Store plasma colour range in menuPalette AND plasmaMenuPalette
	 * so any sub-menu (including in-game file menu) can restore them. */
	{
		int mpi;
		for (mpi = 0; mpi < 16; mpi++) {
			menuPalette[mpi] = palette[mpi];
			plasmaMenuPalette[mpi] = palette[mpi];
		}
	}
