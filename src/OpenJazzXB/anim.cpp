...
#include "jj1level.h"
 ...
 /* jj1level.h -- use xb_anim_compat.h instead */
...
Anim::Anim() {
	...
	sprites = (Sprite**)malloc(19 * sizeof(Sprite*));
	xOffsets = (signed char*)malloc(19 * sizeof(signed char));
	yOffsets = (signed char*)malloc(19 * sizeof(signed char));
...
void Anim::setData(int length, signed char sX, signed char sY, signed char aX, signed char aY, unsigned char a, signed char y) {
	...
	if (length > 19) {
		...
		sprites = (Sprite**)malloc((length) * sizeof(Sprite*));
		xOffsets = (signed char*)malloc((length) * sizeof(signed char));
		yOffsets = (signed char*)malloc((length) * sizeof(signed char));
