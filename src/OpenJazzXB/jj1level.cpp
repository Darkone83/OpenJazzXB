...
JJ1Level::JJ1Level(Game* owner, char* fileName, bool checkpoint, bool multi) :
	...
	/* Xbox: no throw on load fail -- play() will detect broken state */
	(void)ret;
...
int JJ1Level::getLevelNum() {

	return levelNum;

}

void JJ1Level::saveToSlot(int slot) {

	JJ1Save::write(slot, worldNum, levelNum, game->getDifficulty(), setup.characterName);
...
int JJ1Level::play() {
	...
	while (true) {
		...
		/* Handle pending save -- set by Level::select via global */
		if (g_saveSlotPending >= 0) {
			JJ1Save::write(g_saveSlotPending, worldNum, levelNum,
				game->getDifficulty(), setup.characterName);
			g_saveSlotPending = -1;
		}
			...
			if (timeBonus) {
				...
				if (timeBonus == -1) {
					...
					if (nEnemies[+getDifficulty()]) {
						enemyPercent = (levelPlayer->getEnemies() * 100) / nEnemies[+getDifficulty()]; /* was (float) -- int div is exact for 0-100% range */
						if (enemyPercent > 100) enemyPercent = 100;
					}
					if (nItems) {
						itemPercent = (levelPlayer->getItems() * 100) / nItems; /* was (float) */
