...
char* createString(const char* string) {
	...
	char* cloned; cloned = (char*)malloc((strlen(string) + 1) * sizeof(char));
...
char* createString(const char* first, const char* second) {
	...
	char* concatenated; concatenated = (char*)malloc((strlen(first) + strlen(second) + 1) * sizeof(char));
...
char* createFileName(const char* type, int extension) {
	...
	pos = strlen(type); fileName = (char*)malloc((pos + 5) * sizeof(char));
...
char* createFileName(const char* type, const char* extension) {
	...
	pos = strlen(type); fileName = (char*)malloc((pos + strlen(extension) + 2) * sizeof(char));
	strcpy(fileName, type);
	fileName[pos++] = '.';
...
char* createFileName(const char* type, int level, int extension) {
	...
	pos = strlen(type); fileName = (char*)malloc((pos + 6) * sizeof(char));
	strcpy(fileName, type);
...
char* createEditableString(const char* string) {
	...
	char* cloned; cloned = (char*)malloc((STRING_LENGTH + 1) * sizeof(char));
	strcpy(cloned, string);
...
unsigned char* unpackRLE(unsigned char* data, unsigned int size, unsigned int outSize) {
	unsigned char* buffer = (unsigned char*)malloc(outSize * sizeof(unsigned char));
...
void hexDump(const char* desc, const void* addr, const int size, int perLine) {
	/* No printf on Xbox -- OutputDebugStringA only. Hex dump is no-op. */
	(void)desc; (void)addr; (void)size; (void)perLine;
}