...
int JJ1Level::loadTiles(char* fileName) {
	...
	for (int i = 0; i < TSETS; i++) {
		...
		if (marker && strncmp(marker, "ok", 2) == 0) {
...
int JJ1Level::load(char* fileName, bool checkpoint) {
	...
	if (!string) string = createString("CUSTOM");
	...
	if (!ext || !strcmp(ext, "999")) string = createFileName("BLOCKS", worldNum);
