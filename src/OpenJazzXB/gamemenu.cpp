...
int GameMenu::newGameDifficulty(GameModeType mode, char* firstLevel) {
	...
	/* palette[0-15] already has plasma colours from newGameEpisode.
	 * Copy them into menuPalette so plasma is visible here too. */
	for (i = 0; i < 16; i++) menuPalette[i] = palette[i];
	...
	while (true) {
		...
		for (unsigned int i = 0; i < 4; i++) {
			...
			int itemY = (canvasH >> 1) + (i << 4) - 34;

			/* Highlight selected item with a filled rect -- palette-independent.
			 * mapPalette/restorePalette is called before flip() so the remapped
			 * currentPalette is already restored by render time; drawRect has no
			 * such timing issue. */
			if (i == +difficulty)
				video.drawRect((canvasW >> 1) - 62, itemY, 124, 16, 79, false);
...
int GameMenu::newGameEpisode(GameModeType mode) {
	bool exists[12];
	SDL_Color plasmaPalette[MAX_PALETTE_COLORS];
	Plasma plasma;
	char* check;
	SDL_Rect dst;
	int episode, count, x, y, i;

	memcpy(plasmaPalette, video.getPalette(), MAX_PALETTE_COLORS * sizeof(SDL_Color));
	for (i = 0; i < 16; i++) palette[i] = plasmaPalette[i];
	video.setPalette(palette);
