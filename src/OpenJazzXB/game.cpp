...
int Game::playLevel(char* fileName, bool intro, bool checkpoint) {
	...
	if (isFileType(fileName, "macro", 5)) {
		...
		/* Pre-check: if the level file is missing, bail cleanly instead of locking */
		{
			File* testFile = new File(fileName, PATH_TYPE_GAME);
			bool exists = testFile->isOpen();
			delete testFile;
			if (!exists) {
				LOG_WARN("Level file not found: %s", fileName);
				return E_FILE;
			}
		}
		...
		if (intro) {
			...
			/* id==-1: file missing; id==previous: already here -- skip approach */
			if (planet && planet->getId() != -1 && planet->getId() != planetId) {
