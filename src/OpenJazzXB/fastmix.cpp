...
/* Xbox RXDK: no __ftol2_sse -- inline x87 float-to-int (each asm on its own line) */
...
CzCUBICSPLINE::CzCUBICSPLINE()
{
	...
	for (_LIi = 0; _LIi < _LLen; _LIi++)
	{
		...
		lut[_LIdx + 0] = (signed short)Ftoi((_LCm1 < -_LScale) ? -_LScale : ((_LCm1 > _LScale) ? _LScale : _LCm1));
		lut[_LIdx + 1] = (signed short)Ftoi((_LC0 < -_LScale) ? -_LScale : ((_LC0 > _LScale) ? _LScale : _LC0));
		lut[_LIdx + 2] = (signed short)Ftoi((_LC1 < -_LScale) ? -_LScale : ((_LC1 > _LScale) ? _LScale : _LC1));
		lut[_LIdx + 3] = (signed short)Ftoi((_LC2 < -_LScale) ? -_LScale : ((_LC2 > _LScale) ? _LScale : _LC2));
...
CzWINDOWEDFIR::CzWINDOWEDFIR()
{
	...
	for (_LPcl = 0; _LPcl < WFIR_LUTLEN; _LPcl++)
	{
		...
		for (_LCc = 0; _LCc < WFIR_WIDTH; _LCc++)
		{
			...
			lut[_LIdx + _LCc] = (signed short)Ftoi((_LCoef < -_LScale) ? -_LScale : ((_LCoef > _LScale) ? _LScale : _LCoef));
		}
	}
}
