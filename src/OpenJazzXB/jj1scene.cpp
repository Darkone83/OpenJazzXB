...
JJ1Scene::JJ1Scene(const char* fileName) {
	...
	if (!file->isOpen()) {
		LOG_WARN("Scene file not found: %s", fileName);
		return;
	}

	// Checking scene file
	char* identifier1 = file->loadString(18);
	if (!identifier1) return;
...
int JJ1Scene::play() {
	...
	/* Scene may not have loaded (file missing) - bail cleanly */
	if (scriptItems == 0 || pages.empty()) return E_NONE;
	...
	while (true) {
		...
		/* Xbox: clear canvas each frame -- old content persists otherwise */
		video.clearScreen(0);
		...
		if (pages[sceneIndex].backgrounds > 0) {
			...
			if (currentFrame == NULL) {
				...
				if (sceneAnimation != animations.end() && sceneAnimation->background) {
					...
					dst.y = (canvasH - SH) >> 1;
					{
						int _aspd = pages[sceneIndex].animSpeed >> 8;
						frameDelay = (_aspd > 0) ? (1000 / _aspd) : 100; /* guard div/0 */
					}
		...
		for (const auto& sceneText : pages[sceneIndex].texts) {
			...
			if (sceneFont != fonts.end()) {
				...
				if (sceneText.text) {
					...
					// Drop shadow -- write index 0 (black) directly, bypassing
					// palette remapping which has no effect on 8-bit->8-bit blits.
					if (sceneText.shadowColour != -1) {
						font->showSceneStringShadow(sceneText.text, xOffset - 1, yOffset + 2);
					}
